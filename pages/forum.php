...
if($action == '')
{
    ...
    $info = $SQL->query("SELECT " . $SQL->fieldName('section') . ", COUNT(" . $SQL->fieldName('id') . ") AS 'threads', SUM(" . $SQL->fieldName('replies') . ") AS 'replies' FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->fieldName('first_post') . " = " . $SQL->fieldName('id') . " GROUP BY " . $SQL->fieldName('section') . "")->fetchAll();
    foreach($info as $data)
        $counters[$data['section']] = array('threads' => $data['threads'], 'posts' => $data['replies'] + $data['threads']);
    foreach($sections as $id => $section)
    {
        $last_post = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . " = ".(int) $id." AND " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " ORDER BY " . $SQL->fieldName('post_date') . " DESC LIMIT 1")->fetch();
...
if($action == 'show_board')
{
    ...
    $threads_count = $SQL->query("SELECT COUNT(" . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ") AS threads_count FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . " = ".(int) $section_id." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . "")->fetch();
    ...
    $last_threads = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_text') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('last_post') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('replies') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('views') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . " = ".(int) $section_id." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " ORDER BY " . $SQL->tableName('forum') . "." . $SQL->fieldName('last_post') . " DESC LIMIT ".$threads_per_page." OFFSET ".($page * $threads_per_page))->fetchAll();
    if(isset($last_threads[0]))
    {
        ...
        foreach($last_threads as $thread)
        {
            ...
            {
                $last_post = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread['id']." AND " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " ORDER BY " . $SQL->fieldName('post_date') . " DESC LIMIT 1")->fetch();
                if(isset($last_post['name']))
                    $main_content .= date('d.m.y H:i:s', $last_post['post_date']).'<br />by <a href="?subtopic=characters&name='.urlencode($last_post['name']).'">'.$last_post['name'].'</a>';
                else
                    $main_content .= 'No posts.';
...
if($action == 'show_thread')
{
    ...
    $page = (int) $_REQUEST['page'];
    $thread_name = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . " FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread_id." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " AND " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " LIMIT 1")->fetch();
    if(!empty($thread_name['name']))
    {
        $posts_count = $SQL->query("SELECT COUNT(" . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ") AS posts_count FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread_id)->fetch();
        ...
        $threads = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . ", " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('players') . "." . $SQL->fieldName('account_id') . ", " . $SQL->tableName('players') . "." . $SQL->fieldName('vocation') . ", " . $SQL->tableName('players') . "." . $SQL->fieldName('level') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . "," . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . "," . $SQL->tableName('forum') . "." . $SQL->fieldName('post_text') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_smile') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_aid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('last_edit_aid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('edit_date') . ", IFNULL(" . $SQL->tableName('guild_membership') . "." . $SQL->fieldName('rank_id') . ", 0) AS rank_id FROM " . $SQL->tableName('forum') . ", " . $SQL->tableName('players') . " LEFT JOIN " . $SQL->tableName('guild_membership') . " ON (" . $SQL->tableName('guild_membership') . "." . $SQL->fieldName('player_id') . " = " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . ") WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread_id." ORDER BY " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " LIMIT ".$posts_per_page." OFFSET ".($page * $posts_per_page))->fetchAll();
        if(isset($threads[0]['name']))
            $SQL->query("UPDATE " . $SQL->tableName('forum') . " SET " . $SQL->fieldName('views') . "=" . $SQL->fieldName('views') . "+1 WHERE " . $SQL->fieldName('id') . " = ".(int) $thread_id);
        ...
        foreach($threads as $thread)
        {
            ...
            $posts = $SQL->query("SELECT COUNT(" . $SQL->fieldName('id') . ") AS 'posts' FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->fieldName('author_aid') . "=".(int) $thread['account_id'])->fetch();
...
if($action == 'remove_post')
{
    ...
    {
        $id = (int) $_REQUEST['id'];
        $post = $SQL->query("SELECT " . $SQL->fieldName('id') . ", " . $SQL->fieldName('first_post') . ", " . $SQL->fieldName('section') . " FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->fieldName('id') . " = ".$id." LIMIT 1")->fetch();
        if($post['id'] == $id)
        {
            if($post['id'] == $post['first_post'])
            {
                $SQL->query("DELETE FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->fieldName('first_post') . " = ".$post['id']);
                header('Location: ?subtopic=forum&action=show_board&id='.$post['section']);
            }
            else
            {
                $post_page = $SQL->query("SELECT COUNT(" . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ") AS posts_count FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " < ".$id." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $post['first_post'])->fetch();
                $page = (int) ceil($post_page['posts_count'] / $threads_per_page) - 1;
				$SQL->query("UPDATE " . $SQL->tableName('forum') . " SET " . $SQL->fieldName('replies') . " = " . $SQL->fieldName('replies') . " - 1 WHERE " . $SQL->fieldName('id') . " = ".$post['first_post']);
                $SQL->query("DELETE FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->fieldName('id') . " = ".$post['id']);
...
if($action == 'new_post')
{
    if($logged)
    {
        if(canPost($account_logged) || $group_id_of_acc_logged >= $group_not_blocked)
        {
            ...
            $thread = $SQL->query("SELECT " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . " FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " = ".(int) $thread_id." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread_id." LIMIT 1")->fetch();
            ...
            if(isset($thread['id']))
            {
                ...
                {
                    $quoted_post = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_text') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " = ".(int) $quote)->fetchAll();
                ...
                elseif(isset($_REQUEST['save']))
                {
                    ...
                    if(count($errors) == 0)
                    {
						...
						$account_logged->save();
                        $SQL->query("INSERT INTO " . $SQL->tableName('forum') . " (" . $SQL->fieldName('first_post') . " ," . $SQL->fieldName('last_post') . " ," . $SQL->fieldName('section') . " ," . $SQL->fieldName('replies') . " ," . $SQL->fieldName('views') . " ," . $SQL->fieldName('author_aid') . " ," . $SQL->fieldName('author_guid') . " ," . $SQL->fieldName('post_text') . " ," . $SQL->fieldName('post_topic') . " ," . $SQL->fieldName('post_smile') . " ," . $SQL->fieldName('post_date') . " ," . $SQL->fieldName('last_edit_aid') . " ," . $SQL->fieldName('edit_date') . ", " . $SQL->fieldName('post_ip') . ") VALUES ('".$thread['id']."', '0', '".$thread['section']."', '0', '0', '".$account_logged->getId()."', '".(int) $char_id."', ".$SQL->quote($text).", ".$SQL->quote($post_topic).", '".(int) $smile."', '".time()."', '0', '0', '".$_SERVER['REMOTE_ADDR']."')");
                        $SQL->query("UPDATE " . $SQL->tableName('forum') . " SET " . $SQL->fieldName('replies') . "=" . $SQL->fieldName('replies') . "+1, " . $SQL->fieldName('last_post') . "=".time()." WHERE " . $SQL->fieldName('id') . " = ".(int) $thread_id);
                        $post_page = $SQL->query("SELECT COUNT(" . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ") AS posts_count FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " <= ".time()." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread['id'])->fetch();
                        $page = (int) ceil($post_page['posts_count'] / $threads_per_page) - 1;
                ...
                if(!$saved)
                {
                    ...
                    $threads = $SQL->query("SELECT " . $SQL->tableName('players') . "." . $SQL->fieldName('name') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_text') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_smile') . " FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread_id." ORDER BY " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " DESC LIMIT 10")->fetchAll();
...
if($action == 'edit_post')
{
    if($logged)
    {
        if(canPost($account_logged) || $group_id_of_acc_logged >= $group_not_blocked)
        {
            ...
            $thread = $SQL->query("SELECT " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_aid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_text') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_smile') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . " FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " = ".(int) $post_id." LIMIT 1")->fetch();
            if(isset($thread['id']))
            {
                $first_post = $SQL->query("SELECT " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_aid') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_topic') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_text') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_smile') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ", " . $SQL->tableName('forum') . "." . $SQL->fieldName('section') . " FROM " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . " = ".(int) $thread['first_post']." LIMIT 1")->fetch();
                ...
                if($account_logged->getId() == $thread['author_aid'] || $group_id_of_acc_logged >= $group_not_blocked)
                {
                    ...
                    if(isset($_REQUEST['save']))
                    {
                        ...
                        if(count($errors) == 0)
                        {
                            ...
                            $SQL->query("UPDATE " . $SQL->tableName('forum') . " SET " . $SQL->fieldName('author_guid') . " = ".(int) $char_id.", " . $SQL->fieldName('post_text') . " = ".$SQL->quote($text).", " . $SQL->fieldName('post_topic') . " = ".$SQL->quote($post_topic).", " . $SQL->fieldName('post_smile') . " = ".(int) $smile.", " . $SQL->fieldName('last_edit_aid') . " = ".(int) $account_logged->getId()."," . $SQL->fieldName('edit_date') . " = ".time()." WHERE " . $SQL->fieldName('id') . " = ".(int) $thread['id']);
                            $post_page = $SQL->query("SELECT COUNT(" . $SQL->tableName('forum') . "." . $SQL->fieldName('id') . ") AS posts_count FROM " . $SQL->tableName('players') . ", " . $SQL->tableName('forum') . " WHERE " . $SQL->tableName('players') . "." . $SQL->fieldName('id') . " = " . $SQL->tableName('forum') . "." . $SQL->fieldName('author_guid') . " AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('post_date') . " <= ".$thread['post_date']." AND " . $SQL->tableName('forum') . "." . $SQL->fieldName('first_post') . " = ".(int) $thread['first_post'])->fetch();
                            $page = (int) ceil($post_page['posts_count'] / $threads_per_page) - 1;
            ...
            if(isset($sections[$section_id]))
            {
                ...
                if(isset($_REQUEST['save']))
                {
                    ...
                    if(count($errors) == 0)
                    {
                        ...
                        $account_logged->set('last_post', time());
						$account_logged->save();
                        $SQL->query("INSERT INTO " . $SQL->tableName('forum') . " (" . $SQL->fieldName('first_post') . " ," . $SQL->fieldName('last_post') . " ," . $SQL->fieldName('section') . " ," . $SQL->fieldName('replies') . " ," . $SQL->fieldName('views') . " ," . $SQL->fieldName('author_aid') . " ," . $SQL->fieldName('author_guid') . " ," . $SQL->fieldName('post_text') . " ," . $SQL->fieldName('post_topic') . " ," . $SQL->fieldName('post_smile') . " ," . $SQL->fieldName('post_date') . " ," . $SQL->fieldName('last_edit_aid') . " ," . $SQL->fieldName('edit_date') . ", " . $SQL->fieldName('post_ip') . ") VALUES ('0', '".time()."', '".(int) $section_id."', '0', '0', '".$account_logged->getId()."', '".(int) $char_id."', ".$SQL->quote($text).", ".$SQL->quote($post_topic).", '".(int) $smile."', '".time()."', '0', '0', '".$_SERVER['REMOTE_ADDR']."')");
                        $thread_id = $SQL->lastInsertId();
                        $SQL->query("UPDATE " . $SQL->tableName('forum') . " SET " . $SQL->fieldName('first_post') . "=".(int) $thread_id." WHERE " . $SQL->fieldName('id') . " = ".(int) $thread_id);
